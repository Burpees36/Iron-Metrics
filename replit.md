...
-   **Source Discovery & Staging Architecture (Sprint 1)**: Foundation for a connector platform. Connector-agnostic capability discovery probes Wodify endpoints to determine API key access, record shapes, and field types. Tables: `sourceProfiles` (stores profile status, discovered/blocked/empty endpoints, endpoint summaries, field candidates for identifiers/dates/revenue/status, confidence level, recommended next action), `rawStagedPayloads` (stores raw API response pages with deduplication via payload hash, connector-agnostic for future integrations). Backend: `server/field-profiler.ts` (reusable field detection for identifiers, dates, revenue, status fields), `server/wodify-discovery.ts` (probes 10 Wodify endpoints: clients, memberships, classes, attendance, reservations, programs, locations, invoices, payments, wods). Routes: `POST /api/gyms/:id/wodify/profile` (trigger), `GET /api/gyms/:id/wodify/profile` (latest profile + staged payload count), `GET /api/gyms/:id/wodify/staged-payloads` (list staged), `GET /api/gyms/:id/wodify/staged-payloads/:payloadId` (summary). Frontend: Data Source Profile card on Wodify integration page with scan button, profile status/confidence badges, endpoint stats, field discovery display, endpoint detail summaries, warnings, and recommended next action.
