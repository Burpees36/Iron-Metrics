...
function generateStrategicBrief(
  ...
  if (drifterCount > 0) {
    ...
    recommendations.push({
      ...
      detail: `Drifters haven't fully disengaged but are losing momentum. A 7-14 day absence is the trigger window — after that, reactivation rates drop sharply. Personal outreach with a specific return plan and coach follow-up within 48 hours of their return.`,
  ...
  if (avgNewMembers < 3 && coreCount >= 3) {
    ...
    recommendations.push({
      ...
      detail: `You're averaging ${avgNewMembers.toFixed(1)} new members/month. A structured monthly Bring-A-Friend week with coach conversion scripts creates a reliable pipeline. Members can invite one person. Coach welcomes the guest by name, and follows up within 24 hours.`,
  ...
  if (avgNetGrowth <= 0 || avgNewMembers < 2) {
    ...
    recommendations.push({
      category: "Acquisition",
      priority: "medium",
      headline: "Stalled acquisition — build an engine to generate consistent leads",
      detail: `Use your media to solve problems and showcase your results. Member highlights, testimonials, and PR celebration videos create organic lead flow. This is a slower burn (60-120 days) but compounds over time. The key is consistency — multiple posts a week featuring real members and real community moments. Authentic stories, not polished marketing.`,
  ...
  if (activeMemberCount < 80) {
    ...
    recommendations.push({
      category: "Acquisition",
      priority: "medium",
      headline: `Smaller roster — local partnerships are your highest-conversion growth lever`,
      detail: `At ${activeMemberCount} members, community connections are your best growth channel. Local businesses, first responders, schools, sport clubs, and churches are natural partners. Offer a free team class, host a joint event, or create a corporate rate. Word-of-mouth carries further in smaller communities — this is high-confidence acquisition.`,
  ...
  if (isOpenSeason || (cohort3to6Drop && cohort3to6Drop.lostPct > 10) || gymChurnRate > 5) {
    ...
    recommendations.push({
      ...
      detail: isOpenSeason
        ? `The Open transforms "I do CrossFit" into "I AM a CrossFitter." Run Friday Night Lights, set up intramural teams, and make it the event of the year. Encourage every member to register regardless of level. The shared experience creates belonging and prevents cancellations months later.`
  ...
  if ((cohort3to6Drop && cohort3to6Drop.lostPct > 8) || gymArm < 150 || drifterCount > 3) {
    ...
    recommendations.push({
      category: "Community Depth",
      priority: "medium",
      headline: "A nutrition challenge is just as much about increasing members health as it is to increase engagement and strengthen relationships. They help by creating accountability, provide more contact points, increase average revenue per member, and can be a gateway to nutrition coaching.",
      detail: `A structured challenge (macro tracking, accountability pods, weekly check-ins) reignites engagement through shared goals and peer support. Challenges create some of the strongest community bonds in a gym.`,
  ...
  if (gymChurnRate > 5 || (cohort3to6Drop && cohort3to6Drop.lostPct > 12)) {
    ...
    recommendations.push({
      ...
      detail: `Review strength cycle balance, skill progression clarity, warmup protocols, and scaling consistency.${cohort3to6Drop && cohort3to6Drop.lostPct > 12 ? ` Your 3-6 month cohort is losing ${cohort3to6Drop.lostPct.toFixed(0)}% — this often signals programming plateau where members stop seeing progress.` : ""} Is there a clear strength cycle? Are members progressing in skills? These fundamentals compound into retention over time.`,
