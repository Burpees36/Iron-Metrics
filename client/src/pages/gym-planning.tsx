...
export default function GymPlanning() {
  const [, params] = useRoute("/gyms/:id/planning");
    ...
    >
      {predLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground" data-testid="predictive-error">
          Unable to load planning data. Import members and recompute metrics first.
        </div>
      ) : (
        <FuturePlanningView
          cohorts={data.cohortIntelligence}
          scenario={data.revenueScenario}
          gymName={gym.name}
          recommendations={data.strategicBrief.recommendations}
