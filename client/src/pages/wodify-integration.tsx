...
interface CanonicalMappingRun {
  ...
  membershipsPromoted: number;
  membershipsBlocked: number;
  attendanceCandidates: number;
  attendancePromoted: number;
...
interface CanonicalMappingData {
  run: CanonicalMappingRun;
  peopleCounts: DomainCounts;
  membershipCounts: DomainCounts;
  attendanceCounts: DomainCounts;
}

interface MappingConfig {
  id: string;
  personEndpoint: string | null;
  personIdentifierField: string | null;
  personFirstNameField: string | null;
  personLastNameField: string | null;
  personEmailField: string | null;
  membershipEndpoint: string | null;
  membershipIdentifierField: string | null;
  membershipStatusField: string | null;
  membershipStartDateField: string | null;
  membershipEndDateField: string | null;
  attendanceEndpoint: string | null;
  attendanceDateField: string | null;
  attendanceClassNameField: string | null;
...
export default function WodifyIntegration() {
  ...
  const { data: canonicalMappingData, refetch: refetchCanonical } = useQuery<CanonicalMappingData | null>({
    queryKey: ["/api/gyms", gymId, "wodify", "canonical-mapping"],
    enabled: status?.connected === true,
    refetchInterval: (query) => {
      const d = query.state.data;
      if (d?.run?.status === "running" || d?.run?.status === "queued") return 3000;
      return false;
    },
  });

  const { data: mappingConfig } = useQuery<MappingConfig | null>({
    queryKey: ["/api/gyms", gymId, "wodify", "mapping-config"],
    enabled: status?.connected === true,
  });

  const canonicalMappingMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", `/api/gyms/${gymId}/wodify/canonical-mapping`);
      if (!res.ok) {
        const body = await res.json();
        throw new Error(body.message || "Mapping failed");
      }
      return res.json();
    },
    onSuccess: () => {
      toast({ title: "Member data mapping started", description: "Organizing and validating member records..." });
      queryClient.invalidateQueries({ queryKey: ["/api/gyms", gymId, "wodify", "canonical-mapping"] });
      queryClient.invalidateQueries({ queryKey: ["/api/gyms", gymId, "wodify", "mapping-config"] });
    },
    onError: (err: Error) => {
  ...
  const isMappingRunning = canonicalMappingData?.run?.status === "running" || canonicalMappingData?.run?.status === "queued";
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">
                  Organize your staged Wodify data into clean, validated member records for analytics.
                </p>
              </div>
              <Button
                size="sm"
                variant="outline"
                data-testid="button-run-canonical-mapping"
                onClick={() => canonicalMappingMutation.mutate()}
                disabled={canonicalMappingMutation.isPending || isMappingRunning || !profileData?.hasProfile}
              >
                {canonicalMappingMutation.isPending || isMappingRunning ? (
                  <><Loader2 className="w-3 h-3 mr-1 animate-spin" />{isMappingRunning ? "Mapping..." : "Starting..."}</>
                ) : (
                  <><TrendingUp className="w-3 h-3 mr-1" />Map Member Data</>
                )}
              </Button>
            </div>
            ...
            {!profileData?.hasProfile && (
              <p className="text-xs text-muted-foreground italic">
                Run a data profile scan first to enable member data mapping.
              </p>
            )}
            ...
            {isMappingRunning && (
              <div className="flex items-center gap-2 p-3 rounded-md bg-blue-500/10 border border-blue-500/20">
                ...
                <p className="text-sm text-blue-600 dark:text-blue-400">Mapping and validating member records...</p>
              </div>
            )}

            {canonicalMappingData && !isMappingRunning && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 flex-wrap">
                  <MappingRunBadge status={canonicalMappingData.run.status} />
                  {canonicalMappingData.run.startedAt && (
                    <span className="text-xs text-muted-foreground">
                      Last run {formatTimeAgo(canonicalMappingData.run.startedAt)}
                    </span>
                  )}
                  {canonicalMappingData.run.totalDurationMs && (
                    <span className="text-xs text-muted-foreground">
                      ({(canonicalMappingData.run.totalDurationMs / 1000).toFixed(1)}s)
                    </span>
                  )}
                  <span className="text-xs text-muted-foreground">
                    {canonicalMappingData.run.stagedPayloadsProcessed} payloads processed
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <DomainCard
                    ...
                    label="Members"
                    counts={canonicalMappingData.peopleCounts}
                    testId="domain-people"
                  />
                  <DomainCard
                    icon={<CreditCard className="w-4 h-4 text-purple-500" />}
                    label="Memberships"
                    counts={canonicalMappingData.membershipCounts}
                    testId="domain-memberships"
                  />
                  <DomainCard
                    icon={<CalendarDays className="w-4 h-4 text-green-500" />}
                    label="Attendance"
                    counts={canonicalMappingData.attendanceCounts}
                    testId="domain-attendance"
                  />
                </div>
                ...
                {mappingConfig && (
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                      Active Field Mapping {mappingConfig.autoGenerated && <span className="normal-case">(auto-generated)</span>}
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs" data-testid="section-mapping-config">
                      <div className="p-2 rounded-md border bg-muted/30 space-y-1">
                        <p className="font-medium flex items-center gap-1"><Users className="w-3 h-3" /> Members</p>
                        {mappingConfig.personIdentifierField && <MappingField label="ID" value={mappingConfig.personIdentifierField} />}
                        {mappingConfig.personEmailField && <MappingField label="Email" value={mappingConfig.personEmailField} />}
                        {mappingConfig.personFirstNameField && <MappingField label="First name" value={mappingConfig.personFirstNameField} />}
                        {mappingConfig.personLastNameField && <MappingField label="Last name" value={mappingConfig.personLastNameField} />}
                        {!mappingConfig.personIdentifierField && !mappingConfig.personEmailField && (
                          <p className="text-muted-foreground italic">No fields resolved</p>
                        )}
                      </div>
                      <div className="p-2 rounded-md border bg-muted/30 space-y-1">
                        <p className="font-medium flex items-center gap-1"><CreditCard className="w-3 h-3" /> Memberships</p>
                        {mappingConfig.membershipIdentifierField && <MappingField label="ID" value={mappingConfig.membershipIdentifierField} />}
                        {mappingConfig.membershipStatusField && <MappingField label="Status" value={mappingConfig.membershipStatusField} />}
                        {mappingConfig.membershipStartDateField && <MappingField label="Start" value={mappingConfig.membershipStartDateField} />}
                        {mappingConfig.membershipEndDateField && <MappingField label="End" value={mappingConfig.membershipEndDateField} />}
                        {!mappingConfig.membershipIdentifierField && !mappingConfig.membershipStatusField && (
                          <p className="text-muted-foreground italic">No fields resolved</p>
                        )}
                      </div>
                      <div className="p-2 rounded-md border bg-muted/30 space-y-1">
                        <p className="font-medium flex items-center gap-1"><CalendarDays className="w-3 h-3" /> Attendance</p>
                        {mappingConfig.attendanceDateField && <MappingField label="Date" value={mappingConfig.attendanceDateField} />}
                        {mappingConfig.attendanceClassNameField && <MappingField label="Class" value={mappingConfig.attendanceClassNameField} />}
                        {!mappingConfig.attendanceDateField && !mappingConfig.attendanceClassNameField && (
                          <p className="text-muted-foreground italic">No fields resolved</p>
                        )}
                      </div>
                    </div>
                ...
                {canonicalMappingData.run.topBlockers && canonicalMappingData.run.topBlockers.length > 0 && (
                  <div className="space-y-2" data-testid="section-top-blockers">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Top Validation Issues</p>
                    <div className="space-y-1">
                      {canonicalMappingData.run.topBlockers.map((b) => (
                        <div key={b.reason} className="flex items-center justify-between text-xs p-2 rounded-md bg-muted/30 border">
                          <span className="font-mono text-muted-foreground">{b.reason.replace(/_/g, " ")}</span>
                ...
                {canonicalMappingData.run.recommendedNextAction && (
                  <div className="p-3 rounded-md bg-muted/50 border" data-testid="text-canonical-recommended-action">
                    <p className="text-sm font-medium mb-1">Recommended Next Action</p>
                    <p className="text-sm text-muted-foreground">{canonicalMappingData.run.recommendedNextAction}</p>
                  </div>
                )}

                {canonicalMappingData.run.errorMessage && (
                  <div className="p-3 rounded-md bg-destructive/10 border border-destructive/20">
                    <p className="text-xs text-destructive font-medium">Error</p>
                    <p className="text-xs text-destructive/80">{canonicalMappingData.run.errorMessage}</p>
                  </div>
                )}
              </div>
            )}

            {!canonicalMappingData && profileData?.hasProfile && (
              <p className="text-sm text-muted-foreground">
                No mapping has been run yet. Click "Map Member Data" to organize your staged data into validated member records.
              </p>
            )}
          </CardContent>
        </Card>
...
function MappingRunBadge({ status }: { status: string }) {
  switch (status) {
    case "completed":
      return <Badge variant="default" className="text-xs bg-green-600 gap-1"><CheckCircle2 className="w-3 h-3" />Mapping Complete</Badge>;
    case "running":
    case "queued":
      return <Badge variant="default" className="text-xs bg-blue-600 gap-1"><Loader2 className="w-3 h-3 animate-spin" />Running</Badge>;
    case "failed":
...
function DomainCard({ icon, label, counts, testId }: {
  icon: React.ReactNode;
  label: string;
  counts: { total: number; promoted: number; blocked: number; candidate: number };
  testId?: string;
}) {
  return (
    <div className="p-3 rounded-md border bg-muted/30 space-y-2" data-testid={testId}>
      <div className="flex items-center gap-1.5">
        {icon}
        <span className="text-sm font-medium">{label}</span>
        <span className="ml-auto text-base font-semibold tabular-nums">{counts.total}</span>
      </div>
      <div className="space-y-1 text-xs">
        <div className="flex items-center justify-between">
          <span className="text-green-600 dark:text-green-400">Promoted</span>
          <span className="font-semibold tabular-nums" data-testid={`${testId}-promoted`}>{counts.promoted}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-muted-foreground">Pending</span>
          <span className="font-semibold tabular-nums" data-testid={`${testId}-candidate`}>{counts.candidate}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-destructive">Blocked</span>
          <span className="font-semibold tabular-nums" data-testid={`${testId}-blocked`}>{counts.blocked}</span>
        </div>
      </div>
    </div>
  );
...
function MappingField({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-1">
      ...
      <Badge variant="secondary" className="text-[10px] font-mono truncate max-w-[100px]" title={value}>{value}</Badge>
    </div>
  );
}
